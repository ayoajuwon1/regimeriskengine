...
import { buildAnalysisMetadata, buildMarketContext, getIndicatorSeries } from "./contextBuilder.js";
...
test("buildMarketContext resolves core indicators from BLS and Treasury without optional API keys", async (t) => {
  ...
  global.fetch = async (url, options = {}) => {
    if (String(url).startsWith("https://api.bls.gov/publicAPI/v2/timeseries/data/")) {
      ...
      return Response.json({
        ...
        Results: {
          series: [{ seriesID: seriesId, data: seriesMap[seriesId] || [] }],
        },
      });
    }
    ...
    if (String(url).startsWith("https://api.fiscaldata.treasury.gov/services/api/fiscal_service/v1/accounting/od/auctions_query")) {
      ...
      const value = securityTerm === "10-Year" ? "4.20" : "3.85";
      const prior = securityTerm === "10-Year" ? "4.05" : "3.70";
      return Response.json({
        data: [
          { auction_date: "2026-01-15", high_yield: prior },
          { auction_date: "2026-02-15", high_yield: value },
        ],
      });
    }
  ...
  try {
    ...
    assert.equal(context.sourcesUsed.includes("BLS"), true);
    assert.equal(context.sourcesUsed.includes("U.S. Treasury Fiscal Data"), true);
    assert.equal(context.indicatorValues.headline_cpi_yoy.formatted, "3.1%");
    assert.equal(context.indicatorValues.yield_curve_10y_2y.formatted, "0.35%");

    const metadata = buildAnalysisMetadata(context);
    assert.equal(metadata.contextVersion, "public-data-v1");
    assert.equal(metadata.dataSources.length >= 2, true);
  } finally {
...
test("getIndicatorSeries returns derived series for yield curve", async () => {
  const originalFetch = global.fetch;
  delete process.env.FRED_API_KEY;
  delete process.env.BEA_API_KEY;
  ...
  global.fetch = async (url) => {
    if (String(url).startsWith("https://api.fiscaldata.treasury.gov/services/api/fiscal_service/v1/accounting/od/auctions_query")) {
      const securityTerm = new URL(String(url)).searchParams.get("filter")?.split("eq:")[1];
      const value = securityTerm === "10-Year" ? "4.20" : "3.90";
      return Response.json({
        data: [
          { auction_date: "2026-01-15", high_yield: "4.00" },
          { auction_date: "2026-02-15", high_yield: value },
        ],
      });
    }
    ...
    if (String(url).startsWith("https://api.bls.gov/publicAPI/v2/timeseries/data/")) {
      return Response.json({ status: "REQUEST_SUCCEEDED", Results: { series: [{ data: [] }] } });
    }
  ...
  try {
    const result = await getIndicatorSeries("yield_curve_10y_2y");
    assert.equal(result.series.latestFormatted, "0.30%");
  } finally {
    global.fetch = originalFetch;
  }
});
